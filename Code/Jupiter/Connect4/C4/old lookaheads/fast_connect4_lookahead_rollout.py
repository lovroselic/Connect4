...
# https://github.com/MarkusThill/BitBully
...
# Rollout-at-leaf settings. These are intentionally modest:
# the goal is to reduce horizon errors, not to turn every frontier node
# into a full extra search.
C4_ROLLOUT_PLIES = 10
C4_ROLLOUT_ENABLED = True
...
def _ensure_numba_cache():
    ...
    MIRROR_COL = np.array([6, 5, 4, 3, 2, 1, 0], dtype=np.int8)
    ...
    WIN_KIND = np.array(_WIN_KIND, dtype=np.int8)
    ...
    @njit(cache=True, fastmath=True)
    def generate_legal_moves(mask_: UINT, CENTER_ORDER_: np.ndarray, TOP_MASK_: np.ndarray) -> (np.ndarray, np.int32):
    ...
    @njit(cache=True, fastmath=True)
    def mirror_col_fn(c: np.int8, cols_i: np.int32) -> np.int8:
        return np.int8(cols_i - 1 - np.int32(c))

    @njit(cache=True, fastmath=True)
    def mirror_bits(bb: UINT, COL_MASK_: np.ndarray, stride_i: np.int32, cols_i: np.int32) -> UINT:
        out = UINT(0)
        for c in range(cols_i):
            col = bb & COL_MASK_[c]
            dc = cols_i - 1 - c
            if dc > c:
                out |= col << UINT((dc - c) * stride_i)
            elif dc < c:
                out |= col >> UINT((c - dc) * stride_i)
            else:
                out |= col
        return out

    @njit(cache=True, fastmath=True)
    def canonicalize_key(pos_: UINT, mask_: UINT, COL_MASK_: np.ndarray, stride_i: np.int32, cols_i: np.int32) -> (UINT, UINT, np.int8):
        pos_m = mirror_bits(pos_, COL_MASK_, stride_i, cols_i)
        mask_m = mirror_bits(mask_, COL_MASK_, stride_i, cols_i)
        mirrored = np.int8(0)
        if mask_m < mask_:
            mirrored = np.int8(1)
            return pos_m, mask_m, mirrored
        if mask_m == mask_ and pos_m < pos_:
            mirrored = np.int8(1)
            return pos_m, mask_m, mirrored
        return pos_, mask_, mirrored

    @njit(cache=True, fastmath=True)
    def generate_non_losing_moves(
        pos_: UINT, mask_: UINT,
        CENTER_ORDER_: np.ndarray, TOP_MASK_: np.ndarray,
        BOTTOM_MASK_: np.ndarray, COL_MASK_: np.ndarray, stride_i: np.int32
    ) -> (np.ndarray, np.int32):
        ...
        # First collect opponent's direct winning moves from the current node.
        # If there is exactly one, we are forced to block it. If there are two or
        # more, the position is already lost tactically and there are no
        # non-losing moves.
        forced_block = np.int8(-1)
        opp_wins = 0
        for i in range(CENTER_ORDER_.shape[0]):
            c = np.int32(CENTER_ORDER_[i])
            if can_play(mask_, c, TOP_MASK_) and is_winning_move(opp, mask_, c, BOTTOM_MASK_, COL_MASK_, stride_i):
                forced_block = np.int8(c)
                opp_wins += 1
                if opp_wins >= 2:
                    return moves, np.int32(0)

        if opp_wins == 1:
            c = np.int32(forced_block)
            mv = play_bit(mask_, c, BOTTOM_MASK_, COL_MASK_)
            nm = mask_ | mv
            my_after = pos_ | mv
            opp_after = nm ^ my_after
            if count_immediate_wins_bits(opp_after, nm, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i) == 0:
                moves[0] = np.int8(c)
                return moves, np.int32(1)
            return moves, np.int32(0)

        # Otherwise keep only moves that do not hand the opponent an immediate win.
        for i in range(CENTER_ORDER_.shape[0]):
            c = np.int32(CENTER_ORDER_[i])
            if not can_play(mask_, c, TOP_MASK_):
                continue
            mv = play_bit(mask_, c, BOTTOM_MASK_, COL_MASK_)
            nm = mask_ | mv
            my_after = pos_ | mv
            opp_after = nm ^ my_after
            if count_immediate_wins_bits(opp_after, nm, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i) == 0:
                moves[count] = np.int8(c)
                count += 1
        return moves, np.int32(count)
    ...
    @njit(cache=True, fastmath=True)
    def threat_order_score(
        pos_: UINT, mask_: UINT, c: np.int32,
        ply_: np.int16,
        tt_move_: np.int8,
        killers_: np.ndarray,
        history_: np.ndarray,
        CENTER_COL_i: np.int32,
        CENTER_ORDER_: np.ndarray,
        TOP_MASK_: np.ndarray,
        BOTTOM_MASK_: np.ndarray,
        COL_MASK_: np.ndarray,
        ...
        # TT move first: this is usually the best cheap predictor once the table
        # starts getting populated.
        if tt_move_ >= np.int8(0) and c == np.int32(tt_move_):
            s += np.int32(2_000_000)

        # Classic killer/history support remains useful, especially when we are
        # not hitting TT entries yet.
        k1 = killers_[ply_, 0]
        k2 = killers_[ply_, 1]
        if c == np.int32(k1):
            s += np.int32(1_000_000)
        elif c == np.int32(k2):
            s += np.int32(500_000)
        s += np.int32(history_[c])

        # Strong center preference survives because it is both good chess-like
        # hygiene and a solid tie-breaker for rollouts/openings.
        dist = c - CENTER_COL_i
        if dist < 0:
            dist = -dist
        s += np.int32(5_000 - 500 * dist)

        # Threat-first ordering:
        # after playing c, count how many win-in-1 threats we create. Moves that
        # create forcing pressure are searched before quiet moves.
        mv = play_bit(mask_, c, BOTTOM_MASK_, COL_MASK_)
        nm = mask_ | mv
        my_after = pos_ | mv
        opp_after = nm ^ my_after
        ...
        s += np.int32(40_000 * my_threats)
        s -= np.int32(8_000 * opp_threats)

        # Bonus for fork creation. Non-losing generation already prevents the
        # most obvious tactical disasters; this bonus helps us search forcing
        # multi-threat lines early.
        if my_threats >= 2:
            s += np.int32(120_000)
    ...
    @njit(cache=True, fastmath=True)
    def order_moves_threat_first(
        pos_: UINT, mask_: UINT,
        moves_in_: np.ndarray, m_in: np.int32,
        ply_: np.int16,
        tt_move_: np.int8,
        killers_: np.ndarray,
        history_: np.ndarray,
        ...
        for i in range(m_in):
            c = np.int32(moves_in_[i])
            moves[i] = np.int8(c)
            scores[i] = threat_order_score(
                pos_, mask_, c, ply_, tt_move_, killers_, history_,
                CENTER_COL_i, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i
            )
    ...
    @njit(cache=True, fastmath=True)
    def tt_lookup(
        pos_: UINT, mask_: UINT, depth: np.int16, alpha: float, beta: float,
        TT_pos_: np.ndarray, TT_mask_: np.ndarray, TT_depth_: np.ndarray,
        TT_flag_: np.ndarray, TT_val_: np.ndarray, TT_move_: np.ndarray,
        COL_MASK_: np.ndarray, stride_i: np.int32, cols_i: np.int32,
    ) -> (bool, float, np.int8, float, float):
        # Canonicalize through mirror symmetry so that left-right reflections hit
        # the same TT bucket. This is especially valuable in the opening and the
        # early midgame, where symmetric positions are common.
        pos_k, mask_k, mirrored = canonicalize_key(pos_, mask_, COL_MASK_, stride_i, cols_i)
        size_mask = np.int32(TT_pos_.shape[0] - 1)
        idx = tt_hash_local(pos_k, mask_k, size_mask)
        for _ in range(32):
            if TT_depth_[idx] == -1:
                return False, 0.0, np.int8(-1), alpha, beta
            if TT_pos_[idx] == pos_k and TT_mask_[idx] == mask_k:
                d = TT_depth_[idx]
                flag = TT_flag_[idx]
                val = TT_val_[idx]
                mv = TT_move_[idx]
                if mv >= np.int8(0) and mirrored != np.int8(0):
                    mv = mirror_col_fn(mv, cols_i)
                if d >= depth:
                    if flag == 0:
    ...
    @njit(cache=True, fastmath=True)
    def tt_store(
        pos_: UINT, mask_: UINT, depth: np.int16, val: float, alpha0: float, beta: float,
        ...
        pos_k, mask_k, mirrored = canonicalize_key(pos_, mask_, COL_MASK_, stride_i, cols_i)
        mv_k = best_mv
        if mv_k >= np.int8(0) and mirrored != np.int8(0):
            mv_k = mirror_col_fn(mv_k, cols_i)
        ...
        TT_pos_[victim] = pos_k
        TT_mask_[victim] = mask_k
        TT_depth_[victim] = depth
        TT_flag_[victim] = flag
        TT_val_[victim] = val
        TT_move_[victim] = mv_k
    ...
    def rollout_policy_move(
        ...
        moves, m = generate_non_losing_moves(pos_, mask_, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i)
        if m == 0:
            legal, ml = generate_legal_moves(mask_, CENTER_ORDER_, TOP_MASK_)
            if ml == 0:
                return np.int8(-1)
            return legal[0]
        ...
        for i in range(m):
            ...
            if dist < 0:
                dist = -dist
            s = 10_000 * int(my_threats) - 2_000 * int(opp_threats) - 100 * dist
            if my_threats >= 2:
                s += 25_000
            if s > best_s:
                best_s = s
    ...
    def rollout_leaf(
        ...
        for _ in range(np.int32(rollout_plies_)):
            # Win-in-1 from the rollout node: always take it.
            for i in range(CENTER_ORDER_.shape[0]):
                c = np.int32(CENTER_ORDER_[i])
                if can_play(cur_mask, c, TOP_MASK_) and is_winning_move(cur_pos, cur_mask, c, BOTTOM_MASK_, COL_MASK_, stride_i):
                    return MATE_SCORE_i - float(cur_ply)

            if cur_mask == FULL_MASK_:
                return 0.0

            moves, m = generate_non_losing_moves(cur_pos, cur_mask, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i)
            if m == 0:
                # Current side has no non-losing move. That means the opponent has
                # a forced immediate tactical shot after any reply.
                return -MATE_SCORE_i + float(cur_ply + 1)

            c = np.int32(rollout_policy_move(cur_pos, cur_mask, CENTER_COL_i, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i))
            mv = play_bit(cur_mask, c, BOTTOM_MASK_, COL_MASK_)
            nm = cur_mask | mv
            cur_pos = nm ^ (cur_pos | mv)
            cur_mask = nm
            cur_ply = np.int16(cur_ply + 1)

        # When the rollout budget expires, fall back to the handcrafted eval.
        return evaluate(
            cur_pos, cur_mask, COL_MASK_, WIN_MASKS_, WIN_KIND_, WIN_B_, WIN_C_, WIN_R_, WARR_,
            DEF_, FN_, FF_, CENTER_MASK_, ODD_MASK_, EVEN_MASK_, parity_bonus_, parity_enabled_,
            root_pos_is_first_, cur_ply, immediate_w_, fork_w_, center_bonus_, vert_mul_,
    ...
    def negamax(
        ...
        hit, val_tt, mv_tt, alpha, beta = tt_lookup(
            pos_, mask_, depth, alpha, beta,
            TT_pos_, TT_mask_, TT_depth_, TT_flag_, TT_val_, TT_move_,
            COL_MASK_, stride_i, cols_i,
        )
        if hit:
            return val_tt, mv_tt

        # Immediate win from the current node.
        for i in range(CENTER_ORDER_.shape[0]):
            c = np.int32(CENTER_ORDER_[i])
            if can_play(mask_, c, TOP_MASK_) and is_winning_move(pos_, mask_, c, BOTTOM_MASK_, COL_MASK_, stride_i):
        ...
        if depth == 0:
            if rollout_enabled_ != np.int8(0):
                return (
                    rollout_leaf(
                        pos_, mask_, ply, rollout_plies_, root_pos_is_first_, parity_enabled_, MATE_SCORE_i,
                        COL_MASK_, WIN_MASKS_, WIN_KIND_, WIN_B_, WIN_C_, WIN_R_, WARR_,
                        DEF_, FN_, FF_, CENTER_MASK_, ODD_MASK_, EVEN_MASK_, parity_bonus_,
                        immediate_w_, fork_w_, center_bonus_, vert_mul_, vert3_ready_bonus_, tempo_w_,
                        CENTER_COL_i, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, stride_i, rows_i, cols_i, FULL_MASK_
                    ),
                    np.int8(-1),
                )
            return (
        ...
        use_moves, use_len = generate_non_losing_moves(pos_, mask_, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i)
        if use_len == 0:
            # Interior-node shortcut: if there is no non-losing move, the node is
            # tactically lost. Returning the loss immediately is one of the main
            # solver-style speedups we borrow here.
            return -MATE_SCORE_i + float(ply + 1), np.int8(-1)

        # Keep your old extra guards as optional secondary filters. Non-losing
        # generation is now the primary safety mechanism; these are just stricter
        ...
        ordered, ordered_len = order_moves_threat_first(
            pos_, mask_, use_moves, use_len, ply, mv_tt,
            killers_, history_, CENTER_COL, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i
        )

        for j in range(ordered_len):
            c = np.int32(ordered[j])
            mv = play_bit(mask_, c, BOTTOM_MASK_, COL_MASK_)
            nm = mask_ | mv
            next_pos = nm ^ (pos_ | mv)
            ...
            child_val, _ = negamax(
                ...
                rollout_enabled_, rollout_plies_,
                ...
                vert3_ready_bonus_, tempo_w_, CENTER_COL_i, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_,
    ...
    def root_select_fixed(
        ...
        legal, L0 = generate_legal_moves(mask_, CENTER_ORDER_, TOP_MASK_)
        if L0 == 0:
            return np.int32(0)
        ...
        # Preferred root candidate set: non-losing moves if they exist, otherwise
        # all legal moves. This way the engine still returns a move in already
        # lost positions instead of shrugging dramatically.
        legal_nl, L = generate_non_losing_moves(pos_, mask_, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i)
        if L == 0:
            legal_nl, L = legal, L0
        ...
        # Keep your cheap root tactical shots. They still make sense and play well
        # with the new non-losing generator.
        if WIN2_CHECK:
            fork_mv = find_fork_move_root(pos_, mask_, legal_nl, np.int32(L), CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i)
            if fork_mv != np.int32(-1):
                return np.int32(fork_mv)
            win2_mv = find_forced_win_in_2_move_root(pos_, mask_, legal_nl, np.int32(L), CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i)
            if win2_mv != np.int32(-1):
                return np.int32(win2_mv)

        best_move = np.int32(legal_nl[0])
        best_val = -1e100
        ...
        ordered, ordered_len = order_moves_threat_first(
            pos_, mask_, legal_nl, np.int32(L), np.int16(0), np.int8(-1),
            killers_, history_, CENTER_COL, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_, COL_MASK_, stride_i
        )

        for i in range(ordered_len):
            c = np.int32(ordered[i])
            bias = 0.0
            r = np.int16(H[c])
            ...
            v, _ = negamax(
                ...
                WARR_, DEF_, FN_, FF_, CENTER_MASK_, ODD_MASK_, EVEN_MASK_,
                parity_bonus_, immediate_w_, fork_w_, center_bonus_, vert_mul_,
                vert3_ready_bonus_, tempo_w_, CENTER_COL, CENTER_ORDER_, TOP_MASK_, BOTTOM_MASK_,
                stride_i, rows_i, cols_i, killers_, history_, FULL_MASK_,
                TT_pos_, TT_mask_, TT_depth_, TT_flag_, TT_val_, TT_move_
            )
            val = -v + bias
    ...
    _NUMBA_C4_CLASS_CACHE = dict(
        ...
        MIRROR_COL=MIRROR_COL,
        has_won=has_won,
        count_immediate_wins_bits=count_immediate_wins_bits,
        count_safe_moves=count_safe_moves,
        generate_legal_moves=generate_legal_moves,
        generate_non_losing_moves=generate_non_losing_moves,
        mirror_bits=mirror_bits,
        evaluate=evaluate,
...
class Connect4Lookahead:
    ...
    # New solver-inspired switches.
    USE_NON_LOSING_GENERATION = True
    ROLLOUT_LEAVES = C4_ROLLOUT_ENABLED
    ROLLOUT_PLIES = C4_ROLLOUT_PLIES
    ...
    def minimax(self, board, depth, maximizing, player, alpha, beta) -> float:
        ...
        v, _ = self._N["negamax"](
            ...
            np.int8(1 if self.ROLLOUT_LEAVES else 0),
            np.int16(self.ROLLOUT_PLIES),
            node_counter, max_nodes,
            float(self.MATE_SCORE),
            self._N["COL_MASK"], self._N["WIN_MASKS"], self._N["WIN_KIND"],
            self._N["WIN_B"], self._N["WIN_C"], self._N["WIN_R"],
            WARR,
            float(self.DEFENSIVE), float(fn), float(ff),
            np.uint64(self._N["CENTER_MASK"]), np.uint64(self._N["ODD_MASK"]), np.uint64(self._N["EVEN_MASK"]),
            float(self.PARITY_BONUS),
            float(self.immediate_w), float(self.fork_w), float(self.CENTER_BONUS),
            float(self.VERT_MUL), float(self.VERT_3_READY_BONUS), float(self.TEMPO_W),
            np.int32(self.CENTER_COL), self._N["CENTER_ORDER"], self._N["TOP_MASK"], self._N["BOTTOM_MASK"],
            np.int32(self.STRIDE), np.int32(self.ROWS), np.int32(self.COLS),
