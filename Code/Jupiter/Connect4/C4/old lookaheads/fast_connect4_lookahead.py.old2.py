...
C4_FORK_W = C4_WIN
C4_DEFENSIVE = 1.5              # 1.5 kaggle proof
C4_FLOATING_NEAR = 0.70 # 0.90
C4_FLOATING_FAR = 0.35  #0.60
C4_CENTER_BONUS = 10.0  #10 - kaggle proof
    ...
    def negamax(
        ...
        for j in range(use_len):
            ...
            child_val, _ = negamax(
                ...
                WARR_,
                DEF_,
                FN_,  # propagate passed-in floating params
                FF_,  # propagate passed-in floating params
                CENTER_MASK_,
                immediate_w_,
                fork_w_,
                center_bonus_,
    ...
    def root_select_fixed(
        ...
        for i in range(L):
            ...
            v, _ = negamax(
                ...
                WARR_,
                DEF_,
                FN_,  # use passed-in floating params
                FF_,  # use passed-in floating params
                CENTER_MASK_,
                immediate_w_,
                fork_w_,
                center_bonus_,
...
class Connect4Lookahead:
    ...
    # Heuristic knobs (single source of truth, no duplicated literals)
    ...
    def __init__(self, weights=None):
        # Correct + consistent:
        # - Use defaults only when weights is None
        # - Copy dict to avoid external mutation surprises
        if weights is None:
    ...
    def n_step_lookahead(self, board, player, depth=3) -> int:
        ...
        # Ply 0: empty board, we always open center
        if stones == 0:
            return self.CENTER_COL

        # Ply 1: exactly one stone on board (we move second)
        if stones == 1 and self._can_play_py(mask, self.CENTER_COL):
            return self.CENTER_COL
        # ---------------------------------------
    ...
    def policy_scores_delta(self, board, player, depth=1) -> np.ndarray:
        """Like your visualization: action_scores - current_board_score."""
