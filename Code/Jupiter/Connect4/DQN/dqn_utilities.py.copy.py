...
def handle_phase_change(agent, new_phase, current_phase, epsilon, memory_prune_low, epsilon_min, prev_frozen_opp, env, episode, device, Lookahead, training_session, guard_prob, center_start, seed_fraction):
...
def _plot_bench_on_axis(
    ...
    • smooth_k in {0,1} → RAW-ONLY: scatter all points, no line, last-point label
    ...
    for label, ys in history.get("by_opponent", {}).items():
        ...
        if ks:
            # --- smoothed-only ---
            y_s = smooth_same(y, ks[0])
            line, = ax.plot(x, y_s, linewidth=1.8, label=label, solid_capstyle="round")
            col = line.get_color()
            for k in ks[1:]:
                ax.plot(x, smooth_same(y, k), linewidth=2.2, solid_capstyle="round", color=col)

            # last raw dot + label so latest reading is visible
            ax.scatter([x[-1]], [y[-1]], s=22, color=col, zorder=3)
            ax.annotate(f"{y[-1]:.2f}", (x[-1], y[-1]),
                        textcoords="offset points", xytext=(6, -6),
                        ha="left", fontsize=8)
        else:
            # --- raw-only: scatter all points, no line ---
            sc = ax.scatter(x, y, s=14, alpha=0.95, label=label)
            # last-value label
            ax.annotate(f"{y[-1]:.2f}", (x[-1], y[-1]),
                        textcoords="offset points", xytext=(6, -6),
                        ha="left", fontsize=8)
...
def plot_live_training(
    ...
    if use_openings:
        nrows = 12
        base_heights = [3.8, 3.4, 2.6, 1.8, 2.6, 3.2, 2.1, 2.1, 2.1, 2.1, 2.6, 3.0]
    else:
        nrows = 10
        base_heights = [3.8, 3.4, 2.6, 1.8, 2.6, 3.2, 2.1, 2.1, 2.1, 2.1]
    ...
    # Benchmarks
    _plot_bench_on_axis(ax_bench, bench_history, smooth_k=bench_smooth_k, training_phases=None, epsilon_history=epsilon_history, epsilon_min_history=epsilon_min_history)
    ax_bench.set_xlabel("Episode")

    # Action-mix triplet + opponent ε-branch
    ax_mix_explore = fig_ep.add_subplot(gs[6, 0])
    ax_mix_guard   = fig_ep.add_subplot(gs[7, 0])
    ax_mix_eps     = fig_ep.add_subplot(gs[8, 0])
    ax_opp_eps     = fig_ep.add_subplot(gs[9, 0])
    _plot_action_mix_triplet(ax_mix_explore, ax_mix_guard, ax_mix_eps, agent)
    _plot_opp_epsdetail(ax_opp_eps, agent)

    # Openings (hist + rate)
    if use_openings:
        ax_hist = fig_ep.add_subplot(gs[10, 0])
        ax_rate = fig_ep.add_subplot(gs[11, 0], sharex=ax_reward)
