...
class DQNAgent:
    ...
    def act(self, state: np.ndarray, valid_actions, depth: int, strategy_weights=None, ply=None):
        ...
        board = state[0] - state[1]  # mover(+1) − opp(−1)
    
        # --- ε-greedy first (pure exploration: uniform) ---
        if random.random() < self.epsilon:
            self._bump_stat("rand")
            self._bump_stat("epsilon")
            return int(random.choice(valid_actions))
    
        # --- exploitation path only below ---
    
        # 1-ply win, if available
        ...
        # optional center start (only on ply 0/1)
        if not self.center_forced_used and ply is not None and ply <= 1:
            if (random.random() < self.center_start) and (3 in valid_actions) and self._center_col_is_empty(state):
                self.center_forced_used = True
                self._bump_stat("center")
                return 3
    
        # tactical guard (only on exploitation, within the boost window)
        if ply is not None and (self.guard_boost_start <= ply <= self.guard_boost_end) and (random.random() < float(self.guard_prob)):
            must_play, safe_actions = self._tactical_guard(board, valid_actions)
            if must_play is not None:
                self._bump_stat("guard")
                return must_play
            if safe_actions and len(safe_actions) < len(valid_actions):
                self._bump_stat("guard")
                valid_actions = safe_actions
    
        # exploitation: symmetry-averaged Q
