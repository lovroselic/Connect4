...
class DQN(nn.Module):
    """
    Dueling DQN with directional backbone.
    Input: (B, 4, 6, 7) = [me, opp, row, col] with row/col originally in [0,1].
    We zero-center coord planes to [-1,1] and append 2 zero-centered diagonal planes.
    """
    def __init__(self, input_shape=(6, 7), output_size=7, use_diag4: bool = True):
        super().__init__()
        self.H, self.W = input_shape

        # --- precompute *zero-centered* diagonal buffers in [-1,1] ---
        H, W = self.H, self.W
        r = torch.arange(H).view(1, 1, H, 1).float()          # 0..H-1
        c = torch.arange(W).view(1, 1, 1, W).float()          # 0..W-1
        denom = (H - 1) + (W - 1)
        diag_diff = (r - c) / max(1.0, float(max(H - 1, W - 1)))   # ~[-1,1]
        diag_sum  = ((r + c) / max(1.0, float(denom))) * 2.0 - 1.0 # [0,1] -> [-1,1]
        ...
        self._init_weights(output_size)
    ...
    def _append_diagonals(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B = x.shape[0]
        d1 = self.diag_diff.expand(B, -1, -1, -1)  # [-1,1]
        d2 = self.diag_sum.expand(B, -1, -1, -1)   # [-1,1]
        return torch.cat([x, d1, d2], dim=1)

    def _init_weights(self, output_size: int):
        # Kaiming for feature extractors
        ...
        # Neutral heads:
        #  - Advantage last layer = 0 so A(s,·) == 0 initially (no action preference)
        #  - Value last layer small bias to avoid all-zero Q (helps stability)
        with torch.no_grad():
            self.adv_head[-1].weight.zero_()
            if self.adv_head[-1].bias is not None:
                self.adv_head[-1].bias.zero_()

            self.val_head[-1].weight.mul_(0.1)
            if self.val_head[-1].bias is not None:
                self.val_head[-1].bias.fill_(0.05)

    # ---------- forward ----------
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, 4, 6, 7)  [me, opp, row, col], row/col are assumed in [0,1]
        # zero-center coord planes to [-1,1] before appending diagonals
        x = x.clone()
        x[:, 2] = x[:, 2] * 2.0 - 1.0   # row
        x[:, 3] = x[:, 3] * 2.0 - 1.0   # col

        x = self._append_diagonals(x)        # (B, 6, 6, 7) with all coord planes ~[-1,1]
        x = self.backbone(x)                 # (B, 64, 2, 3)
        x = self.gap(x).flatten(1)           # (B, 64)
        V = self.val_head(x)                 # (B, 1)
        A = self.adv_head(x)                 # (B, 7)  -> initially zeros
